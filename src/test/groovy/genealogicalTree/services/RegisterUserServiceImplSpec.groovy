...
class RegisterUserServiceImplSpec extends Specification {
    ...
    def "should save a user successfully when login is unique"() {
        given:
            def userModel = new UserDto(login: "uniqueLOGIN", password: "plainPassword")
            ...
            def mappedUserModel = new UserDto(id: savedUser.id, login: savedUser.login, password: savedUser.password, roles: savedUser.roles)
        ...
        and:
            ...
            userMapper.mapRegisterUserToUserDto(_) >> mappedUserModel
    ...
    def "should throw NotUniqueLogin when login is not unique"() {
        given:
            def userModel = new UserDto(login: "testUser", password: "plainPassword")
            def registerUser = new RegisterUser(login: userModel.login, password: userModel.password)
        and:
            userMapper.mapUserDtoToRegisterUser(userModel) >> registerUser
